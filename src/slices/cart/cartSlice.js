...
export const cartSlice = createSlice({
    ...
    reducers: {
      ...
      removeCart:(state,action) => { // remove o item baseando-se no id passado
        state.items = state.items.filter(item=>{
            return item.id !== action.payload.id
        })
      },
      incrementItem:(state,action) =>{ // incrementa a quantidade do item baseando-se no id passado
      ...
      decrementItem:(state,action) =>{ // incrementa a quantidade do item baseando-se no id passado
          state = {...state,items:state.items.map(item=>{
              return item.id === action.payload.id 
                  ?
