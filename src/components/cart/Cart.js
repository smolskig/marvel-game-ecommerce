...
import { getCart,incrementItem, decrementItem, removeCart } from "../../slices/cart/cartSlice";
...
function Cart() {
  ...
  const displayCartMenu = () => {
    if (cart.items.length > 0) {
        ...
        return (
          <div key={item.id} className="item-list" id="cartRel">
            ...
            <div className="quantity-control" id="cartRel">
              <span onClick={() => handleDecrement(item)} id="cartRel" className="quantity" ><FiMinus className="svg-ico"></FiMinus></span>
              <span onClick={() => dispatch(incrementItem({ id:item.id }))} className="quantity" id="cartRel"><FiPlus className="svg-ico "></FiPlus></span>
            </div>
          </div>
        ) 
      ...
      return <div className="emptyCart">Você não possui nenhum item no carrinho :(</div>
    }
  };
  useEffect(() => {
  ...
  return (
    <>
      <span className="cart icon" id="cartRel" onClick={() => dispatch(toggleCartMenu({ cartOpen: !ui.cartMenuOpen }))}>
              ...
              <div className="info-buy" id="cartRel">
                  ...
                  <div className="button-row" id="cartRel">
                    <span id="cartRel" className="description">
                      Ao clicar no botão de finalizar compra, você será redirecionado
